...
def IPUnit_PYNQ(_data_vec,
    ...
    for i in _weights_vec:
        ...
        _cc = allocate((len(i),), dtype=data_types["data_bus_type"])
        _cc[:] = buff_vec[:]

        # for cnt, j in enumerate(i):
        #     _cc[cnt] = j
        if print_debug is True:
            print("\n\n", type(buff_vec), len(buff_vec))
        weights_vec.append(_cc)

    # convert type(src_data)
    data_vec = []
    for i in _data_vec:
        buff_vec = i
        if "q" in data_types["data_type"]:
    ...
    for control_code in control_code_list:
        ...
        if load_weight != -1:
            ...
            if opcodes[str(opcode)] == "Add":
                ...
                weight = data_vec[load_weight_index][:len_src]
            else:
        ...
        if load_data != -1:
            ...
            src_data = data_vec[src_data_index][:len_src]
        ...
        if store_data != -1:
            ...
            dst_data = data_vec[dst_data_index][:len_dst]
...
def IPUnit_PYNQ_execute_time(_data_vec,
                             opcodes,
                             control_code_file="control_code.bin",
                             weights_vec_file="Weights.bin",
                             bitstream_file="./design_1.bit",
