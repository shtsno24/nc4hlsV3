...
import sys
import copy
sys.path.append("../../../../new_src/Layer/UpSampling2D/Python")
...
def LayerLogWriter(f, layer, threads=None, step=None, init_flag=False):
    ...
    if init_flag is True:
        f.write("# UpSampling Decoder Test  \n\n")
...
def LayerTest(threads, layer,
              ...
              file_name="./log/result_upsampling2d_layer_python.md"):
    ...
    with open(file_name, "w") as f:
        ...
        """
        f.write("# UPSAMPLING2D_LAYER_TEST\n\n")
...
def DecoderTest(threads, decoder,
                ...
                kernel_height, kernel_width,
                file_name="./log/result_upsampling2d_decoder_python.md"):
    mode = 0
    vecs = ["ADDR_SRC_VEC", "ADDR_DST_VEC",
            "ISVALID_SRC_VEC"]
    _loops = int(np.ceil(src_height * src_width * src_depth / threads) * kernel_height * kernel_width)
    ...
    with open(file_name, "w") as f:
        ...
        """
        f.write("# UPSAMPLING2D_DECODER_TEST\n\n")
        mode = 2
...
def DecoderLogWriter(f, decoder, height=None, width=None, depth=None, kernel_height=None, kernel_width=None, threads=None, step=None, init_flag=False):
    if init_flag is True:
        f.write("# UpSampling Decoder Test  \n\n")

        f.write("## TestConstrain  \n\n")
        for i in ["height", "width", "depth", "kernel_height", "kernel_width", "threads"]:
            exec("f.write('{} : ' + str({}) + '  \\n')".format(i, i))

        f.write("\n## DecoderConstants  \n\n")
        for i in ["THREAD_NUM_REG", "LIMIT_HW_IN_REG", "LIMIT_D_IN_REG", "LIMIT_HW_OUT_REG", "LIMIT_D_OUT_REG", "HEIGHT_IN_REG", "WIDTH_IN_REG", "DEPTH_IN_REG", "HEIGHT_OUT_REG", "WIDTH_OUT_REG", "DEPTH_OUT_REG", "KERNEL_HEIGHT_REG", "KERNEL_WIDTH_REG"]:
...
def IntegrationTest(threads, decoder, layer,
                    ...
                    kernel_height, kernel_width,
                    file_name="./log/result_upsampling2d_python.md"):
    mode = 0
    kernel_size = kernel_height * kernel_width
    src_limit = src_height * src_width * src_depth
    dst_limit = dst_height * dst_width * dst_depth
    _loops = ((src_limit + threads - 1) // threads) * kernel_size
...
if __name__ == "__main__":
    try:
        ...
        height_out = height * kernel_height
        width_out = width * kernel_width
        depth_out = depth

        # Decoder Test
        UpSampling2DDecoder = UpSampling2D.Decoder(threads,
                                                   height, width, depth,
                                                   kernel_height, kernel_width)
        DecoderTest(threads, UpSampling2DDecoder,
                    height, width, depth,
                    kernel_height, kernel_width)

        # Layer Test
        UpSampling2DLayer = UpSampling2D.Layer(threads)
        LayerTest(threads, UpSampling2DLayer,
                  kernel_height, kernel_width)

        # Integration Test
        IntegrationTest(threads, UpSampling2DDecoder, UpSampling2DLayer,
                        height, width, depth,
        ...
        for t in range(1, threads + 1):
            loops = int(np.ceil(height * width * depth / t) * kernel_height * kernel_width)
            mem_write = np.array([[-999 for x in range(depth_out)] for y in range(height_out * width_out)])

            UpSamplingDecoder = UpSampling2D.Decoder(t, height, width, depth, kernel_height, kernel_width)
            UpSamplingLayer = UpSampling2D.Layer(t)

            with open("./log/UpSamplingLayer_" + str(t) + ".md", "w") as fl:
                with open("./log/UpSamplingMemory_" + str(t) + ".md", "w") as fm:
                    with open("./log/UpSamplingDecoder_" + str(t) + ".md", "w") as fd:
                        data_in = [0 for x in range(t)]
                        data_out = [0 for x in range(t)]
                        UpSamplingDecoder.decoder(2)
                        DecoderLogWriter(fd, UpSamplingDecoder, height=height, width=width, depth=depth, kernel_height=kernel_height, kernel_width=kernel_width, threads=t, step=-1, init_flag=True)
                        UpSamplingLayer.layer(2, data_in, data_out, UpSamplingDecoder.INVALID_THREAD_IN_VEC)
                        LayerLogWriter(fl, UpSamplingLayer, threads=t, step=-1, init_flag=True)
                        for j in range(loops):
                            UpSamplingDecoder.decoder(1)
                            UpSamplingLayer.INVALID_THREAD_IN_VEC = UpSamplingDecoder.INVALID_THREAD_IN_VEC
                            UpSamplingLayer.INVALID_THREAD_OUT_VEC = UpSamplingDecoder.INVALID_THREAD_OUT_VEC
                            mem_addr_hw_in = list(zip(*UpSamplingDecoder.MEM_ADDR_IN_VEC))[0]
                            mem_addr_d_in = list(zip(*UpSamplingDecoder.MEM_ADDR_IN_VEC))[1]
                            mem_addr_hw_out = list(zip(*UpSamplingDecoder.MEM_ADDR_OUT_VEC))[0]
                            mem_addr_d_out = list(zip(*UpSamplingDecoder.MEM_ADDR_OUT_VEC))[1]

                            for i in range(t):
                                if mem_addr_d_in[i] >= 0 and mem_addr_hw_in[i] >= 0:
                                    data_in[i] = mem_read[mem_addr_hw_in[i]][mem_addr_d_in[i]]

                            data_out = UpSamplingLayer.layer(1, data_in, data_out, UpSamplingDecoder.INVALID_THREAD_IN_VEC)

                            for i in range(t):
                                if mem_addr_d_out[i] >= 0 and mem_addr_hw_out[i] >= 0:
                                    mem_write[mem_addr_hw_out[i]][mem_addr_d_out[i]] = data_out[i]

                            DecoderLogWriter(fd, UpSamplingDecoder, step=j)
                            LayerLogWriter(fl, UpSamplingLayer, step=j)
