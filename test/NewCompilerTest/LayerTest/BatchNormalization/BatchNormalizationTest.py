...
import sys
sys.path.append("../../../../new_src/Layer/BatchNormalization/Python")
...
def LayerTest(threads, layer,
              ...
              file_name="./log/result_batchnormalization_layer_python.md"):

    vecs = ["DATA_SRC_VEC", "DATA_DST_VEC",
            "DATA_WEIGHT_VEC", "DATA_BIAS_VEC",
            "DATA_MOV_AVE_VEC", "DATA_MOV_VAR_VEC",
            "ISVALID_SRC_VEC"]
    ...
    with open(file_name, "w") as f:
        ...
        """
        f.write("# BATCHNORMALIZATION_LAYER_TEST\n\n")
        ...
        DATA_MOV_AVE_VEC = [t for t in range(threads)]
        DATA_MOV_VAR_VEC = [t for t in range(threads)]
...
def LayerLogWriter(f, layer, threads=None, step=None, init_flag=False):
    ...
    if init_flag is True:
        f.write("# BatchNormalization Decoder Test  \n\n")
    ...
    for i in ["INVALID_THREAD_IN_VEC", "INVALID_THREAD_OUT_VEC", "INVALID_THREAD_PARAMETER_VEC", "INVALID_THREAD_BIAS_VEC", "INVALID_THREAD_MOV_AVE_VEC", "INVALID_THREAD_MOV_VAR_VEC", "LAYER_STATUS_REG"]:
        exec("f.write('layer.{} : ' + str(layer.{}) + '  \\n')".format(i, i))

    f.write("\n## LayerDataValiables(step={})  \n\n".format(step))
    for i in ["DATA_IN_VEC", "DATA_OUT_VEC", "PARAMETER_VEC", "BIAS_VEC", "MOV_AVE_VEC", "MOV_VAR_VEC"]:
        exec("f.write('layer.{} : ' + str(layer.{}) + '  \\n')".format(i, i))
...
def DecoderTest(threads, decoder,
                ...
                file_name="./log/result_batchnormalization_decoder_python.md"):
    mode = 0
    vecs = ["ADDR_SRC_VEC", "ADDR_DST_VEC",
            "ADDR_WEIGHT_VEC", "ADDR_BIAS_VEC",
            "ADDR_MOV_AVE_VEC", "ADDR_MOV_VAR_VEC",
            "ISVALID_SRC_VEC"]
    dst_limit = dst_height * dst_width * dst_depth
    _loops = ((dst_limit + threads - 1) // threads)
    ...
    with open(file_name, "w") as f:
        ...
        """
        f.write("# BATCHNORMALIZATION_DECODER_TEST\n\n")
        mode = 2
...
def target_model(input_shape=(), dst_depth=0, kernel_shape=(), strides=()):
    ...
    inputs = [x]
    _x = layers.BatchNormalization()(x)
    outputs = [_x]
...
def IntegrationTest(threads, decoder, layer,
                    ...
                    dst_height, dst_width, dst_depth,
                    file_name="./log/result_batchnormalization_python.md"):
    mode = 0
    src_limit = src_height * src_width * src_depth
    dst_limit = dst_height * dst_width * dst_depth
    gamma_limit = dst_depth
    beta_limit = dst_depth
    ave_limit = dst_depth
    var_limit = dst_depth
    _loops = ((dst_limit + threads - 1) // threads)
    test_memory = TestMemory(threads, src_limit, dst_limit)
    rescale_memory = TestMemory(threads, gamma_limit, beta_limit)
    std_memory = TestMemory(threads, ave_limit, var_limit)
    ...
    _gamma_mem = [0 for x in range(gamma_limit)]
    _beta_mem = [0 for x in range(beta_limit)]
    _ave_mem = [0 for x in range(ave_limit)]
    _var_mem = [0 for x in range(var_limit)]
    ...
    _addr_gamma_mem = [x for x in range(gamma_limit)]
    _addr_beta_mem = [x for x in range(beta_limit)]
    _addr_ave_mem = [x for x in range(ave_limit)]
    _addr_var_mem = [x for x in range(var_limit)]
    test_memory.write_src(_addr_src_mem, SRC_MEM)
    rescale_memory.write_src(_addr_gamma_mem, _gamma_mem)
    rescale_memory.write_dst(_addr_beta_mem, _beta_mem)
    std_memory.write_src(_addr_ave_mem, _ave_mem)
    std_memory.write_dst(_addr_var_mem, _var_mem)
    ...
    with open(file_name, "w") as f:
        ...
        """
        f.write("# BATCHNORMALIZATION_TEST\n\n")
        ...
        DATA_WEIGHT_VEC = rescale_memory.read_src(decoder.ADDR_WEIGHT_VEC)
        DATA_BIAS_VEC = rescale_memory.read_dst(decoder.ADDR_BIAS_VEC)
        DATA_MOV_AVE_VEC = std_memory.read_src(decoder.ADDR_MOV_AVE_VEC)
        DATA_MOV_VAR_VEC = std_memory.read_dst(decoder.ADDR_MOV_VAR_VEC)
    ...
    gamma = np.array(_gamma_mem, dtype=np.int16)
    beta = np.array(_beta_mem, dtype=np.int16)
    moving_average = np.array(_ave_mem, dtype=np.int16)
    moving_variance = np.array(_var_mem, dtype=np.int16)

    bn.set_weights([gamma, beta, moving_average, moving_variance])
    ...
    for i in range(dst_height * dst_width * dst_depth):
        print(DST_MEM[i], test_output[i], ((DST_MEM[i] - test_output[i]) ** 2 < 1e-10))
...
if __name__ == "__main__":
    try:
        ...
        # Decoder Test
        BatchNormalizationDecoder = BatchNormalization.Decoder(threads, height, width, depth,
                                                               kernel_height, kernel_width,
                                                               stride_height, stride_width)
        DecoderTest(threads, BatchNormalizationDecoder,
                    height_out, width_out, depth_out)

        # Layer Test
        BatchNormalizationLayer = BatchNormalization.Layer(threads)
        LayerTest(threads, BatchNormalizationLayer,
                  kernel_height, kernel_width)

        # Integration Test
        IntegrationTest(threads, BatchNormalizationDecoder, BatchNormalizationLayer,
                        height, width, depth,
                        height_out, width_out, depth_out)
