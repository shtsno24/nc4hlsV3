...
import sys
sys.path.append("../../../../new_src/Layer/AveragePooling2D/Python")
...
def LayerTest(threads, layer,
              ...
              file_name="./log/result_averagepooling2d_layer_python.md"):
    ...
    with open(file_name, "w") as f:
        ...
        """
        f.write("# AVERAGEPOOLING2D_LAYER_TEST\n\n")
        ...
        DATA_WEIGHT_VEC = [[kernel_size for t in range(threads)] for k in range(kernel_size)]
        ...
        ISVALID_SRC_VEC[-1] = [3 for t in range(threads)]
...
def LayerLogWriter(f, layer, threads=None, step=None, init_flag=False):
    ...
    if init_flag is True:
        f.write("# AveragePooling2D Decoder Test  \n\n")
...
def DecoderTest(threads, decoder,
                ...
                kernel_height, kernel_width,
                file_name="./log/result_averagepooling2d_decoder_python.md"):
    mode = 0
    ...
    with open(file_name, "w") as f:
        ...
        """
        f.write("# AVERAGEPOOLING2D_DECODER_TEST\n\n")
        mode = 2
...
def target_model(input_shape=(), dst_depth=0, kernel_shape=(2, 2), strides=(2, 2)):
    """
        sample code :
    ...
    inputs = [x]
    _x = layers.AveragePooling2D(pool_size=kernel_shape)(x)
    outputs = [_x]
...
def IntegrationTest(threads, decoder, layer,
                    ...
                    dst_height, dst_width, dst_depth,
                    kernel_height, kernel_width,
                    file_name="./log/result_averagepooling2d_python.md"):
    mode = 0
    kernel_size = kernel_height * kernel_width
    weight_limit = 1
    ...
    weight_memory = TestMemory(threads, 1, 1)
    ...
    for ch in range(weight_limit):
        _weight_mem[ch] = kernel_size
...
if __name__ == "__main__":
    try:
        ...
        # Decoder Test
        AveragePooling2DDecoder = AveragePooling2D.Decoder(threads, height, width, depth,
                                                           kernel_height, kernel_width,
                                                           stride_height, stride_width)
        DecoderTest(threads, AveragePooling2DDecoder,
                    height_out, width_out, depth_out,
                    kernel_height, kernel_width)

        # Layer Test
        AveragePooling2DLayer = AveragePooling2D.Layer(threads)
        LayerTest(threads, AveragePooling2DLayer,
                  kernel_height, kernel_width)

        # Integration Test
        IntegrationTest(threads, AveragePooling2DDecoder, AveragePooling2DLayer,
                        height, width, depth,
                        height_out, width_out, depth_out,
                        kernel_height, kernel_width)
