...
import sys
import copy
sys.path.append("../../../../new_src/Layer/ZeroPadding2D/Python")
...
def LayerTest(threads, layer,
              ...
              file_name="./log/result_zeropadding2d_layer_python.md"):
    ...
    with open(file_name, "w") as f:
        ...
        """
        f.write("# ZEROPADDING2D_LAYER_TEST\n\n")
        ...
        ISVALID_SRC_VEC[3] = [2 for t in range(threads)]
...
def DecoderTest(threads, decoder,
                ...
                file_name="./log/result_zeropadding2d_decoder_python.md"):
    mode = 0
    vecs = ["ADDR_SRC_VEC", "ADDR_DST_VEC",
            "ISVALID_SRC_VEC"]
    _loops = int(np.ceil((src_height + pad_top + pad_bottom) * (src_width + pad_left + pad_right) / threads) * src_depth)
    ...
    with open(file_name, "w") as f:
        ...
        """
        f.write("# ZEROPADDING2D_DECODER_TEST\n\n")
        mode = 2
...
def DecoderLogWriter(f, decoder, height=None, width=None, depth=None, pad_top=None, pad_bottom=None, pad_left=None, pad_right=None, threads=None, step=None, init_flag=False):
    if init_flag is True:
        f.write("# NopDepth Decoder Test  \n\n")

        f.write("## TestConstrain  \n\n")
        for i in ["height", "width", "depth", "pad_top", "pad_bottom", "pad_left", "pad_right", "threads"]:
            exec("f.write('{} : ' + str({}) + '  \\n')".format(i, i))

        f.write("\n## DecoderConstants  \n\n")
        for i in ["THREAD_NUM_REG", "LIMIT_HW_IN_REG", "LIMIT_D_IN_REG", "LIMIT_HW_OUT_REG", "LIMIT_D_OUT_REG", "LIMIT_SEL_0_REG", "LIMIT_SEL_1_REG", "LIMIT_SEL_2_REG", "HEIGHT_REG", "WIDTH_REG", "DEPTH_REG", "PAD_TOP_REG", "PAD_BOTTOM_REG", "PAD_LEFT_REG", "PAD_RIGHT_REG"]:
...
def IntegrationTest(threads, decoder, layer,
                    ...
                    pad_height, pad_width, pad_left, pad_right,
                    file_name="./log/result_zeropadding2d_python.md"):
    mode = 0
    src_limit = src_height * src_width * src_depth
    dst_limit = dst_height * dst_width * dst_depth
    _loops = int(np.ceil(dst_height * dst_width / threads) * src_depth)
...
if __name__ == "__main__":
    try:
        ...
        pad_left = 3
        pad_right = 4
        height_out = height + pad_top + pad_bottom
        width_out = width + pad_left + pad_right
        depth_out = depth

        # Decoder Test
        ZeroPadding2DDecoder = ZeroPadding2D.Decoder(threads,
                                                     height, width, depth,
                                                     pad_top, pad_bottom, pad_left, pad_right)
        DecoderTest(threads, ZeroPadding2DDecoder,
                    height, width, depth,
                    pad_top, pad_bottom, pad_left, pad_right)

        # Layer Test
        ZeroPadding2DLayer = ZeroPadding2D.Layer(threads)
        LayerTest(threads, ZeroPadding2DLayer,
                  2, 3)

        # Integration Test
        IntegrationTest(threads, ZeroPadding2DDecoder, ZeroPadding2DLayer,
                        height, width, depth,
        ...
        for t in range(1, threads + 1):
            loops = int(np.ceil((height + pad_top + pad_bottom) * (width + pad_left + pad_right) / t) * depth)
            mem_write = np.array([[-1 for x in range(depth)] for y in range((height + pad_top + pad_bottom) * (width + pad_left + pad_right))])

            ZeroPaddingDecoder = ZeroPadding2D.Decoder(t, height, width, depth, pad_top=pad_top, pad_bottom=pad_bottom, pad_left=pad_left, pad_right=pad_right)
            ZeroPaddingLayer = ZeroPadding2D.Layer(t)

            with open("./log/ZeroPaddingLayer_" + str(t) + ".md", "w") as fl:
                with open("./log/ZeroPaddingMemory_" + str(t) + ".md", "w") as fm:
                    with open("./log/ZeroPaddingDecoder_" + str(t) + ".md", "w") as fd:
                        data_in = [0 for x in range(t)]
                        data_out = [0 for x in range(t)]
                        ZeroPaddingDecoder.decoder(2)
                        DecoderLogWriter(fd, ZeroPaddingDecoder, height=height, width=width, depth=depth, pad_top=pad_top, pad_bottom=pad_bottom, pad_left=pad_left, pad_right=pad_right, threads=t, step=-1, init_flag=True)
                        ZeroPaddingLayer.layer(2, data_in, data_out, ZeroPaddingDecoder.INVALID_THREAD_IN_VEC)
                        LayerLogWriter(fl, ZeroPaddingLayer, threads=t, step=-1, init_flag=True)
                        for j in range(loops):
                            ZeroPaddingDecoder.decoder(1)
                            DecoderLogWriter(fd, ZeroPaddingDecoder, step=j)
                            ZeroPaddingLayer.INVALID_THREAD_IN_VEC = ZeroPaddingDecoder.INVALID_THREAD_IN_VEC
                            ZeroPaddingLayer.INVALID_THREAD_OUT_VEC = ZeroPaddingDecoder.INVALID_THREAD_OUT_VEC
                            mem_addr_hw_in = list(zip(*ZeroPaddingDecoder.MEM_ADDR_IN_VEC))[0]
                            mem_addr_d_in = list(zip(*ZeroPaddingDecoder.MEM_ADDR_IN_VEC))[1]
                            mem_addr_hw_out = list(zip(*ZeroPaddingDecoder.MEM_ADDR_OUT_VEC))[0]
                            mem_addr_d_out = list(zip(*ZeroPaddingDecoder.MEM_ADDR_OUT_VEC))[1]

                            for i in range(t):
                                if mem_addr_d_in[i] >= 0 and mem_addr_hw_in[i] >= 0:
                                    data_in[i] = mem_read[mem_addr_hw_in[i]][mem_addr_d_in[i]]

                            data_out = ZeroPaddingLayer.layer(1, data_in, data_out, ZeroPaddingDecoder.INVALID_THREAD_IN_VEC)

                            for i in range(t):
                                if mem_addr_d_out[i] >= 0 and mem_addr_hw_out[i] >= 0:
                                    mem_write[mem_addr_hw_out[i]][mem_addr_d_out[i]] = data_out[i]

                            LayerLogWriter(fl, ZeroPaddingLayer, step=j)
                            MemoryLogWriter(fm, mem_read, mem_write, (depth, height, width), (depth, (height + pad_top + pad_bottom), (width + pad_left + pad_right)), step=j, init_flag=True if j == 0 else False)
