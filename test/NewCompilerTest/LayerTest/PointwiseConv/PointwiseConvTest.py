...
import sys
import copy
sys.path.append("../../../../new_src/Layer/PointwiseConv/Python")

import PointwiseConv

def LayerTest(threads, layer,
              kernel_height, kernel_width,
              file_name="./log/result_pointwiseconv_layer_python.md"):
    ...
    with open(file_name, "w") as f:
        ...
        """
        f.write("# POINTWISECONV_LAYER_TEST\n\n")
...
def LayerLogWriter(f, layer, threads=None, step=None, init_flag=False):
    ...
    if init_flag is True:
        f.write("# PointwiseConv Decoder Test  \n\n")
...
def DecoderTest(threads, decoder, dst_depth,
                src_height, src_width, src_depth,
                file_name="./log/result_pointwiseconv_decoder_python.md"):
    mode = 0
    vecs = ["ADDR_SRC_VEC", "ADDR_DST_VEC",
            "ADDR_WEIGHT_VEC", "ADDR_BIAS_VEC",
            "ISVALID_SRC_VEC"]
    _loops = int(np.ceil(src_height * src_width / threads) * src_depth) * dst_depth
    ...
    with open(file_name, "w") as f:
        ...
        """
        f.write("# POINTWISECONV_DECODER_TEST\n\n")
        mode = 2
...
def DecoderLogWriter(f, decoder, height=None, width=None, depth=None, depth_out=None, threads=None, step=None, init_flag=False):
    if init_flag is True:
        f.write("# PointwiseConv Decoder Test  \n\n")

        f.write("## TestConstrain  \n\n")
        for i in ["height", "width", "depth", "depth_out", "threads"]:
            exec("f.write('{} : ' + str({}) + '  \\n')".format(i, i))

        f.write("\n## DecoderConstants  \n\n")
        for i in ["THREAD_NUM_REG", "LIMIT_HW_IN_REG", "LIMIT_D_IN_REG", "LIMIT_HW_OUT_REG", "LIMIT_D_OUT_REG", "HEIGHT_IN_REG", "WIDTH_IN_REG", "DEPTH_IN_REG", "HEIGHT_OUT_REG", "WIDTH_OUT_REG", "DEPTH_OUT_REG"]:
...
def IntegrationTest(threads, decoder, layer,
                    ...
                    kernel_depth, kernel_ch,
                    file_name="./log/result_pointwiseconv_python.md"):
    mode = 0
    kernel_size = kernel_depth * kernel_ch
    src_limit = src_height * src_width * src_depth
    dst_limit = dst_height * dst_width * dst_depth
    weight_limit = kernel_size
    bias_limit = kernel_ch
    _loops = int(np.ceil(src_height * src_width / threads) * src_depth) * dst_depth
    ...
    for ch in range(kernel_ch):
        for d in range(kernel_depth):
            _weight_mem[ch * kernel_depth + d] = (ch + 1) * (d + 1)
        _bias_mem[ch] = (ch + 1)
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Decoder Test
        PointwiseConvDecoder = PointwiseConv.Decoder(threads, height, width, depth, depth_out)
        DecoderTest(threads, PointwiseConvDecoder, depth_out,
                    height, width, depth)

        # Layer Test
        PointwiseConvLayer = PointwiseConv.Layer(threads)
        LayerTest(threads, PointwiseConvLayer,
                  depth, depth_out)

        # Integration Test
        IntegrationTest(threads, PointwiseConvDecoder, PointwiseConvLayer,
                        height, width, depth,
                        height, width, depth_out,
                        depth, depth_out)
        ...
        params = np.array([[0 for x in range(depth)] for y in range(depth_out)])
        for ch in range(depth_out):
            for d in range(depth):
                params[ch][d] = (ch + 1) * (d + 1)
        bias = np.array([x + 1 for x in range(depth_out)])

        for t in range(1, threads + 1):
            loops = int(np.ceil(height * width / t) * depth) * depth_out
            mem_write = np.array([[-1 for x in range(depth_out)] for y in range(height * width)])

            PointwiseConvDecoder = PointwiseConv.Decoder(t, height, width, depth, depth_out)
            PointwiseConvLayer = PointwiseConv.Layer(t)

            with open("./log/PointwiseConvLayer_" + str(t) + ".md", "w") as fl:
                with open("./log/PointwiseConvMemory_" + str(t) + ".md", "w") as fm:
                    with open("./log/PointwiseConvDecoder_" + str(t) + ".md", "w") as fd:
                        data_in = [0 for x in range(t)]
                        data_out = [0 for x in range(t)]
                        _params = [0 for x in range(t)]
                        _bias = [0 for x in range(t)]
                        PointwiseConvDecoder.decoder(2)
                        DecoderLogWriter(fd, PointwiseConvDecoder, height=height, width=width, depth=depth, depth_out=depth_out, threads=t, step=-1, init_flag=True)
                        PointwiseConvLayer.layer(2, data_in, data_out, _params, _bias, PointwiseConvDecoder.INVALID_THREAD_IN_VEC)
                        LayerLogWriter(fl, PointwiseConvLayer, threads=t, step=-1, init_flag=True)
                        for j in range(loops):
                            PointwiseConvDecoder.decoder(1)

                            PointwiseConvLayer.INVALID_THREAD_IN_VEC = PointwiseConvDecoder.INVALID_THREAD_IN_VEC
                            PointwiseConvLayer.INVALID_THREAD_OUT_VEC = PointwiseConvDecoder.INVALID_THREAD_OUT_VEC
                            PointwiseConvLayer.INVALID_THREAD_PARAMETER_VEC = PointwiseConvDecoder.INVALID_THREAD_PARAMETER_VEC
                            PointwiseConvLayer.INVALID_THREAD_BIAS_VEC = PointwiseConvDecoder.INVALID_THREAD_BIAS_VEC

                            mem_addr_hw_in = list(zip(*PointwiseConvDecoder.MEM_ADDR_IN_VEC))[0]
                            mem_addr_d_in = list(zip(*PointwiseConvDecoder.MEM_ADDR_IN_VEC))[1]
                            mem_addr_hw_out = list(zip(*PointwiseConvDecoder.MEM_ADDR_OUT_VEC))[0]
                            mem_addr_d_out = list(zip(*PointwiseConvDecoder.MEM_ADDR_OUT_VEC))[1]

                            param_addr_ch = list(zip(*PointwiseConvDecoder.PARAMETER_ADDR_VEC))[0]
                            param_addr_d = list(zip(*PointwiseConvDecoder.PARAMETER_ADDR_VEC))[1]
                            bias_addr_d = PointwiseConvDecoder.BIAS_ADDR_VEC

                            DecoderLogWriter(fd, PointwiseConvDecoder, step=j)

                            for i in range(t):
                                if 0 <= mem_addr_d_in[i] and mem_addr_hw_in[i] >= 0:
                                    data_in[i] = mem_read[mem_addr_hw_in[i]][mem_addr_d_in[i]]
                                    _params[i] = params[param_addr_ch[i]][param_addr_d[i]]
                                    _bias[i] = bias[bias_addr_d[i]]

                            data_out = PointwiseConvLayer.layer(1, data_in, data_out, _params, _bias, PointwiseConvDecoder.INVALID_THREAD_IN_VEC)

                            for i in range(t):
                                if 0 <= mem_addr_d_out[i] and mem_addr_hw_out[i] >= 0:
                                    mem_write[mem_addr_hw_out[i]][mem_addr_d_out[i]] = data_out[i]

                            LayerLogWriter(fl, PointwiseConvLayer, step=j)
                            MemoryLogWriter(fm, mem_read, mem_write, (depth, height, width), (depth_out, height, width), step=j, init_flag=True if j == 0 else False)
