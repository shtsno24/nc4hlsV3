...
import sys
import copy
sys.path.append("../../../src/Layer/LeakyReLU/Python")
...
def LayerLogWriter(f, layer, threads=None, step=None, init_flag=False):
    ...
    if init_flag is True:
        f.write("# LeakyReLU Decoder Test  \n\n")
...
def LayerTest(threads, layer,
              ...
              file_name="./log/result_leakyrelu_layer_python.md"):
    ...
    with open(file_name, "w") as f:
        ...
        DATA_WEIGHT_VEC = [2 * t for t in range(threads)]
        DATA_BIAS_VEC = [0 for t in range(threads)]
...
def DecoderTest(threads, decoder,
                ...
                file_name="./log/result_leakyrelu_decoder_python.md"):
    mode = 0
    vecs = ["ADDR_SRC_VEC", "ADDR_DST_VEC",
            "ADDR_WEIGHT_VEC", "ADDR_BIAS_VEC",
            "ISVALID_SRC_VEC"]
    _loops = int(np.ceil(src_height * src_width * src_depth / threads))
    ...
    with open(file_name, "w") as f:
        ...
        """
        f.write("# LEAKYRELU_DECODER_TEST\n\n")
        mode = 2
...
def IntegrationTest(threads, decoder, layer,
                    ...
                    file_name="./log/result_leakyrelu_python.md"):
    mode = 0
    ...
    weight_memory.write_src(_addr_weight_mem, WEIGHT_MEM)
    ...
    with open(file_name, "w") as f:
        ...
        layer.layer(mode, DATA_SRC_VEC, DATA_DST_VEC,
                    DATA_WEIGHT_VEC, DATA_WEIGHT_VEC,
                    ISVALID_SRC_VEC)
...
if __name__ == "__main__":
    try:
        ...
        # Decoder Test
        LeakyReLUDecoder = LeakyReLU.Decoder(threads,
                                             height, width, depth)
        DecoderTest(threads, LeakyReLUDecoder,
                    height, width, depth)

        # Layer Test
        LeakyReLULayer = LeakyReLU.Layer(threads)
        LayerTest(threads, LeakyReLULayer,
                  3, 3)

        # Integration Test
        IntegrationTest(threads, LeakyReLUDecoder, LeakyReLULayer,
                        height, width, depth,
                        height, width, depth)
