...
import sys
import copy
sys.path.append("../../../../new_src/Layer/ReLU/Python")
...
def LayerTest(threads, layer,
              ...
              file_name="./log/result_relu_layer_python.md"):
    ...
    with open(file_name, "w") as f:
        ...
        """
        f.write("# RELU_LAYER_TEST\n\n")

        DATA_SRC_VEC = [[(k + 1) * (t + 1) - kernel_size for t in range(threads)] for k in range(kernel_size)]
...
def DecoderTest(threads, decoder,
                ...
                file_name="./log/result_relu_decoder_python.md"):
    mode = 0
    vecs = ["ADDR_SRC_VEC", "ADDR_DST_VEC",
            "ISVALID_SRC_VEC"]
    _loops = int(np.ceil(height * width / threads) * depth)
...
def DecoderLogWriter(f, decoder, height=None, width=None, depth=None, threads=None, step=None, init_flag=False):
    if init_flag is True:
        f.write("# ReLUHW Decoder Test  \n\n")

        f.write("## TestConstrain  \n\n")
        for i in ["height", "width", "depth", "threads"]:
            exec("f.write('{} : ' + str({}) + '  \\n')".format(i, i))

        f.write("\n## DecoderConstants  \n\n")
        for i in ["THREAD_NUM_REG", "LIMIT_HW_IN_REG", "LIMIT_D_IN_REG", "LIMIT_HW_OUT_REG", "LIMIT_D_OUT_REG", "HEIGHT_REG", "WIDTH_REG", "DEPTH_REG"]:
...
def IntegrationTest(threads, decoder, layer,
                    ...
                    file_name="./log/result_relu_python.md"):
    mode = 0
    src_limit = src_height * src_width * src_depth
    dst_limit = dst_height * dst_width * dst_depth
    _loops = int(np.ceil(src_height * src_width / threads) * src_depth)
    ...
    for h in range(src_height):
        for w in range(src_width):
            for d in range(src_depth):
                ...
                SRC_MEM[(h * src_width + w) * src_depth + d] -= src_limit // 2
...
if __name__ == "__main__":
    try:
        ...
        # Decoder Test
        ReLUDecoder = ReLU.Decoder(threads,
                                   height, width, depth)
        DecoderTest(threads, ReLUDecoder,
                    height, width, depth)

        # Layer Test
        ReLULayer = ReLU.Layer(threads)
        LayerTest(threads, ReLULayer,
                  3, 3)

        # Integration Test
        IntegrationTest(threads, ReLUDecoder, ReLULayer,
                        height, width, depth,
                        height, width, depth)
        ...
        for h in range(height):
            for w in range(width):
                for d in range(depth):
                    ...
                    mem_read[h * width + w][d] -= (height * width * depth // 2)

        for t in range(1, threads + 1):
            loops = int(np.ceil(height * width * depth / t))
            mem_write = np.array([[-1 for x in range(depth)] for y in range(height * width)])

            ReLUHWDecoder = ReLU.Decoder(t, height, width, depth)
            ReLUHWLayer = ReLU.Layer(t)

            with open("./log/ReLUHWLayer_" + str(t) + ".md", "w") as fl:
                with open("./log/ReLUHWMemory_" + str(t) + ".md", "w") as fm:
                    with open("./log/ReLUHWDecoder_" + str(t) + ".md", "w") as fd:
                        data_in = [0 for x in range(t)]
                        data_out = [0 for x in range(t)]
                        ReLUHWDecoder.decoder(2)
                        DecoderLogWriter(fd, ReLUHWDecoder, height=height, width=width, depth=depth, threads=t, step=-1, init_flag=True)
                        ReLUHWLayer.layer(2, data_in, data_out, ReLUHWDecoder.INVALID_THREAD_IN_VEC)
                        LayerLogWriter(fl, ReLUHWLayer, threads=t, step=-1, init_flag=True)
                        for j in range(loops):
                            ReLUHWDecoder.decoder(1)
                            ReLUHWLayer.INVALID_THREAD_IN_VEC = ReLUHWDecoder.INVALID_THREAD_IN_VEC
                            ReLUHWLayer.INVALID_THREAD_OUT_VEC = ReLUHWDecoder.INVALID_THREAD_OUT_VEC
                            mem_addr_hw_in = list(zip(*ReLUHWDecoder.MEM_ADDR_IN_VEC))[0]
                            mem_addr_d_in = list(zip(*ReLUHWDecoder.MEM_ADDR_IN_VEC))[1]
                            mem_addr_hw_out = list(zip(*ReLUHWDecoder.MEM_ADDR_OUT_VEC))[0]
                            mem_addr_d_out = list(zip(*ReLUHWDecoder.MEM_ADDR_OUT_VEC))[1]

                            for i in range(t):
                                if mem_addr_d_in[i] >= 0 and mem_addr_hw_in[i] >= 0:
                                    data_in[i] = mem_read[mem_addr_hw_in[i]][mem_addr_d_in[i]]

                            data_out = ReLUHWLayer.layer(1, data_in, data_out, ReLUHWDecoder.INVALID_THREAD_IN_VEC)

                            for i in range(t):
                                if mem_addr_d_out[i] >= 0 and mem_addr_hw_out[i] >= 0:
                                    mem_write[mem_addr_hw_out[i]][mem_addr_d_out[i]] = data_out[i]

                            DecoderLogWriter(fd, ReLUHWDecoder, step=j)
                            LayerLogWriter(fl, ReLUHWLayer, step=j)
                            MemoryLogWriter(fm, mem_read, mem_write, (depth, height, width), (depth, height, width), step=j, init_flag=True if j == 0 else False)
