...
import sys
sys.path.append("../../../../new_src/Layer/Dense/Python")
...
def LayerTest(threads, layer,
              in_shape, out_shape,
              file_name="./log/result_dense_layer_python.md"):

    vecs = ["DATA_SRC_VEC", "DATA_DST_VEC",
            "DATA_WEIGHT_VEC", "DATA_BIAS_VEC",
            "ISVALID_SRC_VEC"]
    kernel_size = in_shape * out_shape
...
def DecoderTest(threads, decoder,
                src_size, dst_size,
                file_name="./log/result_dense_decoder_python.md"):
    mode = 0
    vecs = ["ADDR_SRC_VEC", "ADDR_DST_VEC",
            "ADDR_WEIGHT_VEC", "ADDR_BIAS_VEC",
            "ISVALID_SRC_VEC"]
    dst_limit = dst_size
    _loops = ((dst_limit + threads - 1) // threads) * src_size
...
def target_model(input_shape=(), dst_size=0):
    x = layers.Input(shape=input_shape, name="data_in")
    inputs = [x]
    _x = layers.Dense(dst_size, activation=None)(x)
    outputs = [_x]
...
def IntegrationTest(threads, decoder, layer,
                    src_size,
                    dst_size,
                    file_name="./log/result_dense_python.md"):
    mode = 0
    src_limit = src_size
    dst_limit = dst_size
    weight_limit = src_size * dst_size
    bias_limit = dst_size
    _loops = ((dst_size + threads - 1) // threads) * src_size
    ...
    for d in range(src_size):
        for ch in range(dst_size):
            _weight_mem[d * dst_size + ch] = (d + 1) * (ch + 1)
            _bias_mem[ch] = (ch + 1)
    ...
    with open(file_name, "w") as f:
        ...
        for w in range(src_size):
            f.write("[" if w == 0 else "")
            f.write("{}".format(SRC_MEM[w]))
            f.write("]\n\n" if w == src_size - 1 else ", ")

        f.write("#### DST_MEM (TEST_0, step = -1)\n\n")
        for w in range(dst_size):
            f.write("[" if w == 0 else "")
            f.write("{}".format(DST_MEM[w]))
            f.write("]\n\n" if w == dst_size - 1 else ", ")
    ...
    # run keras model
    model = target_model((src_size), dst_size)
    dense = model.layers[1]
    kernel = np.array(_weight_mem, dtype=np.int32)
    kernel = kernel.reshape(dense.get_weights()[0].shape)
    ...
    test_input = test_input.reshape((1, src_size))
...
if __name__ == "__main__":
    try:
        ...
        # Decoder Test
        DenseDecoder = Dense.Decoder(threads, in_shape, out_shape)
        DecoderTest(threads, DenseDecoder, in_shape, out_shape)

        # Layer Test
        DenseLayer = Dense.Layer(threads)
        LayerTest(threads, DenseLayer,
                  in_shape, out_shape)

        # Integration Test
        IntegrationTest(threads, DenseDecoder, DenseLayer,
                        in_shape, out_shape)
