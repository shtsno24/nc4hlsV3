...
int main(){
    std::ofstream _dump_file("./log/dump_relu.md");
    _dump_file << "# RELU_LAYER_TEST" << std::endl << std::endl;
    std::ofstream _result_file("./log/result_relu_layer.md");
    _result_file << "# RELU_LAYER_TEST" << std::endl << std::endl;

    constexpr int kernel_size = 3 * 3;
     ...
     * run
     */
    for(data_t i = 0; i < 6; i++){
        for(data_t k = 0; k < kernel_size; k++){
            for(data_t t = 0; t < thread_num; t++){
                DATA_SRC_VEC[k][t] = (k + 1) * (t + 1) - kernel_size;
                DATA_DST_VEC[t] = k + t;
                ISVALID_SRC_VEC[k][t] = i;
            }
        }
        #ifdef DUMP_LOG_CMP
        ...
        for(data_t k = 0; k < kernel_size; k++) {   
            ...
            _dump_file << std::endl;
            #else
            cmp::calc_unit<data_t, addr_t, signal_t, thread_num>(DATA_SRC_VEC[k], DATA_DST_VEC, ISVALID_SRC_VEC[k]);
            #endif
