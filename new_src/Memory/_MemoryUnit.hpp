...
#endif
#pragma HLS INLINE
#pragma HLS RESOURCE variable=data_port core=RAM_T2P_BRAM
#pragma HLS dependence variable=data_port inter false
        /*
         * rw_mode
...
#pragma HLS array_partition variable=DATA_READ_VEC complete dim=0
        // define cache
//         static data_t data_cache_a[thread][thread];
//         static addr_t index_cache_a[thread][thread];
...
#pragma HLS RESOURCE variable=bank_a core=RAM_T2P_BRAM
#pragma HLS dependence variable=bank_a inter false
...
#pragma HLS UNROLL
#pragma HLS PIPELINE
#pragma HLS loop_tripcount min=thread max=thread
                // if (ADDR_WRITE_VEC[i] != -1){
...
// #pragma HLS UNROLL
//                         if(index_cache_a[i][j] == ADDR_READ_VEC[i]){
//                             DATA_READ_VEC[i] = data_cache_a[i][j];
//                             hit_cache = true;
//                         }
//                     }
//                 }
        ...
        //         for(uint16_t j = 0; j < thread; j++){
        // #pragma HLS UNROLL
        //             index_cache_a[i][j] = ADDR_READ_VEC[j];
        //             data_cache_a[i][j] = DATA_READ_VEC[j];
        //         }
        //     }
        // }
...
#pragma HLS array_partition variable=DATA_READ_VEC complete dim=0
        // define cache
//         static data_t data_cache_b[thread][thread];
//         static addr_t index_cache_b[thread][thread];
// #pragma HLS RESOURCE variable=data_cache_b core=RAM_2P_LUTRAM
// #pragma HLS RESOURCE variable=index_cache_b core=RAM_2P_LUTRAM
// #pragma HLS array_partition variable=data_cache_b cyclic factor=thread
// #pragma HLS array_partition variable=index_cache_b cyclic factor=thread
...
#pragma HLS RESOURCE variable=bank_b core=RAM_T2P_BRAM
#pragma HLS dependence variable=bank_b inter false
...
//                     for(uint16_t j = 0; j < thread; j++){
// #pragma HLS UNROLL
//                         if(index_cache_b[i][j] == ADDR_READ_VEC[i]){
//                             DATA_READ_VEC[i] = data_cache_b[i][j];
//                             hit_cache = true;
//                         }
//                     }
//                 }
        ...
        //         for(uint16_t j = 0; j < thread; j++){
        // #pragma HLS UNROLL
        //             index_cache_b[i][j] = ADDR_READ_VEC[j];
        //             data_cache_b[i][j] = DATA_READ_VEC[j];
        //         }
        //     }
        // }
...
#pragma HLS RESOURCE variable=bank_w core=RAM_T2P_BRAM
#pragma HLS dependence variable=bank_w inter false
...
#pragma HLS loop_tripcount min=thread max=thread
            ...
            addr_t buff_addr[thread];
#pragma HLS array_partition variable=buff_addr complete dim=0
            for(uint16_t i = 0; i < thread; i++){
// #pragma HLS UNROLL factor=2
#pragma HLS UNROLL
#pragma HLS PIPELINE
#pragma HLS loop_tripcount min=thread max=thread
                buff_addr[i] = ADDR_BIAS_VEC[i] + addr_bias_offset;
                if(addr_bias_offset != -1){
                    if(ADDR_BIAS_VEC[i] != -1){
                        DATA_BIAS_VEC[i] = bank_w[buff_addr[i]];
                    } else {
                        DATA_BIAS_VEC[i] = 0;
                    }
                } else {
...
#pragma HLS PIPELINE
#pragma HLS loop_tripcount min=thread max=thread
                buff_addr[i] = ADDR_MEAN_VEC[i] + addr_mean_offset;
                if(addr_mean_offset != -1){
                    if(ADDR_MEAN_VEC[i] != -1){
                        DATA_MEAN_VEC[i] = bank_w[buff_addr[i]];
                    } else {
                        DATA_MEAN_VEC[i] = 0;
                    }
                } else {
