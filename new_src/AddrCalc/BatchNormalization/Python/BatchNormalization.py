"""
reference
https://qiita.com/satopirka/items/4f6e3702947f7ef8f1bf
https://note.nkmk.me/python-tensorflow-keras-batch-normalization-training/
...
class Layer():
    ...
    def __init__(self, thread_num):
        ...
        self.MOV_AVE_VEC = [0 for x in range(self.THREAD_NUM_REG)]
        self.MOV_VAR_VEC = [0 for x in range(self.THREAD_NUM_REG)]
        ...
        self.INVALID_THREAD_MOV_AVE_VEC = [0 for x in range(self.THREAD_NUM_REG)]  # 0:INVALID, 1:VALID
        self.INVALID_THREAD_MOV_VAR_VEC = [0 for x in range(self.THREAD_NUM_REG)]  # 0:INVALID, 1:VALID
        ...
        self.DATA_MOV_AVE_VEC = [0 for x in range(self.THREAD_NUM_REG)]
        self.DATA_MOV_VAR_VEC = [0 for x in range(self.THREAD_NUM_REG)]
    ...
    def run(self, data_in, data_out, params, bias, mov_ave, mov_var, isvalid):
        ...
        try:
            ...
            # Read moving average from Mov_Ave Bus
            for i in range(self.THREAD_NUM_REG):
                if self.INVALID_THREAD_MOV_AVE_VEC[i] == 1:
                    self.MOV_AVE_VEC[i] = mov_ave[i]
                else:
                    self.MOV_AVE_VEC[i] = 0

            # Read moving variance from Mov_Var Bus
            for i in range(self.THREAD_NUM_REG):
                if self.INVALID_THREAD_MOV_VAR_VEC[i] == 1:
                    self.MOV_VAR_VEC[i] = mov_var[i]
                else:
                    self.MOV_VAR_VEC[i] = 0

            # Execute data
            for i in range(self.THREAD_NUM_REG):
                if self.INVALID_THREAD_IN_VEC[i] == 1:
                    _x = (self.DATA_IN_VEC[i] - self.MOV_AVE_VEC[i]) / ((self.MOV_VAR_VEC[i] + 1e-3) ** 0.5)  # eps=0.001, initial value in keras
                    self.DATA_OUT_VEC[i] = _x * self.PARAMETER_VEC[i] + self.BIAS_VEC[i]
            ...
            for i in range(self.THREAD_NUM_REG):
                ...
                if self.ISVALID_SRC_VEC[i] in [1, 2]:
                    ...
                    self.DATA_MOV_AVE_VEC[i] = mov_ave[i]
                    self.DATA_MOV_VAR_VEC[i] = mov_var[i]
                else:
                    self.DATA_SRC_VEC[i] = -1
                    self.DATA_WEIGHT_VEC[i] = 0
                    self.DATA_BIAS_VEC[i] = 0
                    self.DATA_MOV_AVE_VEC[i] = 0
                    self.DATA_MOV_VAR_VEC[i] = 1

            # Execute data
            for i in range(self.THREAD_NUM_REG):
                if self.ISVALID_SRC_VEC[i] == 1:
                    _x = (self.DATA_SRC_VEC[i] - self.DATA_MOV_AVE_VEC[i]) / ((self.DATA_MOV_VAR_VEC[i] + 1e-3) ** 0.5)
                    self.DATA_DST_VEC[i] = _x * self.DATA_WEIGHT_VEC[i] + self.DATA_BIAS_VEC[i]
    ...
    def layer(self, mode, data_in, data_out, params, bias, mov_ave, mov_var, isvalid):
        if mode == 0:
            self.idle()
        elif mode == 2:
            self.reset()
        elif mode == 1:
            return self.run(data_in, data_out, params, bias, mov_ave, mov_var, isvalid)
...
class Decoder():
    ...
    def __init__(self, thread_num, height=0, width=0, depth=0, kernel_height=0, kernel_width=0, stride_height=1, stride_width=1):
        ...
        self.ADDR_MOV_AVE_VEC = [-1 for x in range(self.THREAD_NUM_REG)]
        self.ADDR_MOV_VAR_VEC = [-1 for x in range(self.THREAD_NUM_REG)]
        self.ISVALID_SRC_VEC = [0 for x in range(self.THREAD_NUM_REG)]  # 0:INVALID, 1:VALID, 2:PATH_THROUGH
        self.STATUS_REG = 0
    ...
    def decoder(self, mode):
        if mode == 2:
            ...
            for i in range(self.THREAD_NUM_REG):
                ...
                mem_addr_d_out = self.CNT_OUT_VEC[i] % self.DEPTH_OUT_REG

                mem_addr_hw_out = self.CNT_OUT_VEC[i] // self.DEPTH_OUT_REG
                mem_addr_hw_in = mem_addr_hw_out
                mem_addr_d_in = mem_addr_d_out

                mem_addr_hw_params = 0
                mem_addr_d_params = mem_addr_d_out
                mem_addr_d_bias = mem_addr_d_out

                self.ADDR_DST_VEC[i] = self.CNT_OUT_VEC[i]
                self.ADDR_SRC_VEC[i] = self.CNT_OUT_VEC[i]
                ...
                if(self.ADDR_DST_VEC[i] < self.LIMIT_OUT_REG):
                    ...
                    self.ADDR_WEIGHT_VEC[i] = mem_addr_d_out
                    self.ADDR_BIAS_VEC[i] = mem_addr_d_out
                    self.ADDR_MOV_AVE_VEC[i] = mem_addr_d_out
                    self.ADDR_MOV_VAR_VEC[i] = mem_addr_d_out

                    self.ISVALID_SRC_VEC[i] = 1  # 0:INVALID, 1:VALID, 2:PATH_THROUGH
                    self.STATUS_REG = 1  # 0:IDLE, 1:RUN, 2:RESET, 3:DONE

                elif(mem_addr_d_out == self.LIMIT_OUT_REG):
                    ...
                    self.ADDR_MOV_AVE_VEC[i] = -1
                    self.ADDR_MOV_VAR_VEC[i] = -1
                    ...
                    self.ADDR_MOV_AVE_VEC[i] = -2
                    self.ADDR_MOV_VAR_VEC[i] = -2
                    self.ISVALID_SRC_VEC[i] = 0  # 0:INVALID, 1:VALID, 2:PATH_THROUGH
