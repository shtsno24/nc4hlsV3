...
int main(){
    ...
    const addr_t weight_limit = dst_depth;
    ...
    addr_t ADDR_MEAN_VEC[thread_num], ADDR_GAMMA_VAR_VEC[thread_num]; //mean, weight
    signal_t mode = 0;  // 0:RESET, 1:RUN
    data_t DATA_SRC_VEC[thread_num];
    data_t DATA_DST_VEC[thread_num];
    data_t DATA_GAMMA_VAR_VEC[thread_num];
    ...
    addr_t _ADDR_GAMMA_VAR[bias_limit];
    addr_t _ADDR_BIAS[bias_limit];
    data_t GAMMA_VAR_MEM[bias_limit];
    data_t BIAS_MEM[bias_limit];

    addr_t _ADDR_MEAN[bias_limit];
    data_t MEAN_MEM[bias_limit];
    addr_t _ADDR_VAR[bias_limit];
    data_t VAR_MEM[bias_limit];
    ...
    for(int ch = 0; ch < dst_depth; ch++){
        GAMMA_VAR_MEM[ch] = (ch + 1) * (1.0 / std::sqrt((ch + 4.0) + 1e-3));
        BIAS_MEM[ch] = (ch + 2);
        MEAN_MEM[ch] = -(ch + 3);
        VAR_MEM[ch] = 0;
    }
    ...
    memory::rom<data_t, addr_t, signal_t, weight_limit, bias_limit> std_memory;

    test_memory.write_src(src_limit, _ADDR_SRC, SRC_MEM);
    test_memory.write_dst(dst_limit, _ADDR_DST, DST_MEM);
    weight_memory.write_src(weight_limit, _ADDR_GAMMA_VAR, GAMMA_VAR_MEM);
    weight_memory.write_dst(bias_limit, _ADDR_BIAS, BIAS_MEM);
    std_memory.write_src(bias_limit, _ADDR_MEAN, MEAN_MEM);
    std_memory.write_dst(bias_limit, _ADDR_VAR, VAR_MEM);

    std::ofstream _result_file("./log/result_batchnormalization.md");
    _result_file << "# BATCHNORMALIZATION_TEST"  << std::endl << std::endl;
    ...
    std::ofstream _dump_file("./log/dump_batchnormalization_decoder_.md");
    ...
    weight_memory.read_src(thread_num, ADDR_GAMMA_VAR_VEC, DATA_GAMMA_VAR_VEC);
    weight_memory.read_dst(thread_num, ADDR_BIAS_VEC, DATA_BIAS_VEC);
    std_memory.read_src(thread_num, ADDR_MEAN_VEC, DATA_MEAN_VEC);
    // std_memory.read_dst(thread_num, ADDR_VAR_VEC, DATA_VAR_VEC);

    // Execute
    #ifdef DUMP_LOG_DSP
    std::ofstream __dump_file("./log/dump_batchnormalization_layer_.md");
