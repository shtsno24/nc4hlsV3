class Layer():
    ...
    def run(self, data_in, data_out, isvalid):
        ...
        try:
            ...
            # Execute data
            for i in range(self.THREAD_NUM_REG):
                self.DATA_OUT_VEC[i] = self.DATA_IN_VEC[i] if self.DATA_IN_VEC[i] > 0 else 0
            ...
            for i in range(self.THREAD_NUM_REG):
                if self.ISVALID_SRC_VEC[i] == 1:
                    self.DATA_DST_VEC[i] = self.DATA_SRC_VEC[i] if self.DATA_SRC_VEC[i] > 0 else 0
    ...
    def decoder(self, mode):
        if mode == 2:
            ...
            for i in range(self.THREAD_NUM_REG):
                ...
                mem_addr_hw_in = self.ADDR_HW_IN(self.CNT_IN_REG, i)
                mem_addr_d_in = self.ADDR_D_IN(self.CNT_IN_REG, i)
                self.CNT_IN_REG += 1
