...
namespace conv2d{
    ...
    void decoder(addr_t src_height, addr_t src_width, addr_t src_depth, addr_t src_limit,
        ...
        static addr_t k_h, k_w, k_d, k_ch, k_hwd;
        static addr_t bias_d, bias_addr, weight_addr;
        static addr_t dst_d, dst_hw, dst_w, dst_h, dst_addr;
        static addr_t src_d, src_w, src_h, src_hw, src_addr;
        static addr_t _cnt_dst_head, dst_d_head, dst_w_head, dst_h_head, dst_hw_head;
...
#pragma HLS UNROLL
                ...
                src_addr = src_hw + k_d;
                weight_addr = k_hwd + dst_d;
                bias_addr = dst_d;

                if (_cnt_kernel == 0){
                    is_valid = 2;
                } else if (_cnt_kernel == const_vec[0]){   // TODO : "(kernel_size * src_depth - 1)" is a const. -> const_vec[0]
                    is_valid = 3;
                } else {
                    is_valid = 1;
                }
                ...
                // calc dst_h, dst_w, dst_d, dst_addr
                _cnt_dst += 1;
                dst_hw += dst_depth;
                if(dst_hw == dst_limit){
                    dst_hw = 0;
                ...
                dump_file << "src_h_head : " << src_h_head << "  " << std::endl;
                dump_file << "src_w_head : " << src_w_head << "  " << std::endl;
            ...
            // k_hwd = ((k_h * kernel_width + k_w) * src_depth + k_d) * dst_depth;
            k_hwd += dst_depth;
            if (k_hwd == const_vec[7]){  // TODO : "src_depth * kernel_size * dst_depth" is a const. -> const_vec[7]
                k_hwd = 0;
            }

            _cnt_kernel += 1;
            if (_cnt_kernel == const_vec[8]){  // TODO : "kernel_size * src_depth" is a const. -> const_vec[8]
                _cnt_kernel = 0;
            }
        } else if (mode == 0){
            /*
