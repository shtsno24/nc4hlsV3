...
constexpr int thread_num = 19;

int main(){
    std::ofstream _dump_file("./log/dump_conv2d_decoder.md");
    ...
    std::ofstream _result_file("./log/result_conv2d_decoder.md");
    _result_file << "# CONV2D_DECODER_TEST"  << std::endl << std::endl;
    ...
    for(int i = 0; i < ((dst_limit + thread_num - 1) / thread_num) * kernel_size * src_depth; i++){
        #ifdef DUMP_LOG_CONV2D
        conv2d::decoder<data_t, addr_t, signal_t, thread_num>(src_height, src_width, src_depth, src_limit,
                                                              dst_height, dst_width, dst_depth, dst_limit,
                                                              kernel_height, kernel_width, kernel_size,
