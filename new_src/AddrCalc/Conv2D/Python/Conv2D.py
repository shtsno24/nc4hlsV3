...
class Decoder():
    ...
    def __init__(self, thread_num, height=0, width=0, depth=0, depth_out=0, kernel_height=0, kernel_width=0, stride_height=1, stride_width=1):
        self.THREAD_NUM_REG = thread_num
    ...
    def decoder(self, mode):
        if mode == 2:
            ...
            for i in range(self.THREAD_NUM_REG):
                ...
                mem_addr_hw_out = self.CNT_OUT_VEC[i] % (self.HEIGHT_OUT_REG * self.WIDTH_OUT_REG)
                mem_addr_d_out = self.CNT_OUT_VEC[i] // (self.HEIGHT_OUT_REG * self.WIDTH_OUT_REG)
                mem_addr_h_out = mem_addr_hw_out // self.WIDTH_OUT_REG
                mem_addr_w_out = mem_addr_hw_out % self.WIDTH_OUT_REG

                mem_addr_hwd_params = self.CNT_IN_REG % (self.KERNEL_HEIGHT_REG * self.KERNEL_WIDTH_REG * self.DEPTH_IN_REG)
                mem_addr_ch_params = mem_addr_d_out
                mem_addr_hw_params = mem_addr_hwd_params // self.DEPTH_IN_REG
                mem_addr_d_params = mem_addr_hwd_params % self.DEPTH_IN_REG
                mem_addr_h_weight = mem_addr_hw_params // self.KERNEL_WIDTH_REG
                mem_addr_w_weight = mem_addr_hw_params % self.KERNEL_WIDTH_REG

                mem_addr_d_bias = mem_addr_ch_params

                mem_addr_d_in = mem_addr_d_params
                mem_addr_h_in = mem_addr_h_out * self.STRIDE_HEIGHT_REG + mem_addr_h_weight
                mem_addr_w_in = mem_addr_w_out * self.STRIDE_WIDTH_REG + mem_addr_w_weight
                mem_addr_hw_in = mem_addr_h_in * self.WIDTH_IN_REG + mem_addr_w_in
                ...
                if(mem_addr_d_out < self.LIMIT_D_OUT_REG):
                    if self.CNT_IN_REG % (self.KERNEL_HEIGHT_REG * self.KERNEL_WIDTH_REG * self.DEPTH_IN_REG) == 0:
                    ...
                    self.ADDR_WEIGHT_VEC[i] = self.ADDR_WEIGHT_VEC[i] * self.DEPTH_IN_REG + mem_addr_d_params
                    self.ADDR_WEIGHT_VEC[i] = self.ADDR_WEIGHT_VEC[i] * self.DEPTH_OUT_REG + mem_addr_ch_params
                ...
                elif(mem_addr_d_out == self.LIMIT_D_OUT_REG):
            ...
            self.CNT_IN_REG += 1
            self.CNT_OUT_REG = self.CNT_IN_REG // (self.KERNEL_HEIGHT_REG * self.KERNEL_WIDTH_REG * self.DEPTH_IN_REG)
