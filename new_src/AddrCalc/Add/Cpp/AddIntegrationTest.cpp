...
int main(){
    ...
    addr_t _ADDR_WEIGHT[src_limit];
    data_t WEIGHT_MEM[src_limit];
    addr_t _ADDR_BIAS[src_limit];
    data_t BIAS_MEM[src_limit];
    ...
    for(int h = 0; h < src_height; h++){
        for(int w = 0; w < src_width; w++){
            for(int d = 0; d < src_depth; d++){
                addr_t _index = (h * src_width + w) * src_depth + d;
                SRC_MEM[_index] = (h + 1) * (w + 1) * (d + 1);
                SRC_MEM[_index] -= (src_height * src_width * src_depth / 2);
                WEIGHT_MEM[_index] = SRC_MEM[_index];
                BIAS_MEM[_index] = 0;
            }
        }
    }
    ...
    memory::rom<data_t, addr_t, signal_t, src_limit, src_limit> weight_memory;

    test_memory.write_src(src_limit, _ADDR_SRC, SRC_MEM);
    test_memory.write_dst(dst_limit, _ADDR_DST, DST_MEM);

    weight_memory.write_src(src_limit, _ADDR_WEIGHT, WEIGHT_MEM);
    weight_memory.write_dst(src_limit, _ADDR_BIAS, BIAS_MEM);

    std::ofstream _result_file("./log/result_add.md");
    _result_file << "# ADD_TEST"  << std::endl << std::endl;
    ...
    #ifdef DUMP_LOG_ADD
    std::ofstream _dump_file("./log/dump_add_decoder_.md");
    _dump_file << "# ADD_DECODER_TEST" << std::endl << std::endl;
    _dump_file << "## TEST_0 (reset)" << std::endl << std::endl;
    add::decoder<data_t, addr_t, signal_t, thread_num>(src_height, src_width, src_depth, src_limit,
    ...
    // Execute
    #ifdef DUMP_LOG_DSP
    std::ofstream __dump_file("./log/dump_add_layer_.md");
