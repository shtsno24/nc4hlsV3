...
int main(){
    ...
    for(int h = 0; h < kernel_height; h++){
        for(int w = 0; w < kernel_width; w++){
            for(int d = 0; d < src_depth; d++){
                WEIGHT_MEM[(h * kernel_width + w) * src_depth + d] = (h + 1) * (w + 1) * (d + 1);
                BIAS_MEM[d] = (d + 1);
            }
        }
    }
    ...
    std::ofstream _result_file("./log/result_depthwiseconv2d.md");
    _result_file << "# DEPTHWISECONV2D_TEST"  << std::endl << std::endl;
    ...
    std::ofstream _dump_file("./log/dump_depthwiseconv2d_decoder_.md");
    ...
    // Execute
    #ifdef DUMP_LOG_DSP
    std::ofstream __dump_file("./log/dump_dsp.md");
